from collections import Counter; from itertools import chain; import operator; import string; import requests
ALLOWABLE_CHARACTERS = set(string.ascii_letters); ALLOWED_ATTEMPTS = 6; WORD_LENGTH = 5
def get_dictionary():
    res = requests.get("https://raw.githubusercontent.com/dwyl/english-words/master/words_dictionary.json"); word_list = list(res.json().keys())
    return {word.lower() for word in word_list if len(word) == WORD_LENGTH and set(word) < ALLOWABLE_CHARACTERS}
...
def calculate_word_commonality(word):
    score = 0.0
    for char in word: score += LETTER_FREQUENCY[char]
...
def display_word_table(word_commonalities):
    for (word, freq) in word_commonalities: print(f"{word:<10} | {freq:<5.2}")
def input_word():
    while True:
        word = input("Input the word you entered> ")
        if len(word) == WORD_LENGTH and word.lower() in WORDS: break
    return word.lower()
def input_response():
    print("Type the color-coded reply from Wordle:"); print("  G for Green"); print("  Y for Yellow"); print("  ? for Gray")
    while True:
        response = input("Response from Wordle> ")
        if len(response) == WORD_LENGTH and set(response) <= {"G", "Y", "?"}: break
        else: print(f"Error - invalid answer {response}")
    return response
...
def solve():
    ...
    for attempt in range(1, ALLOWED_ATTEMPTS + 1):
        ...
        for idx, letter in enumerate(response):
            if letter == "G": word_vector[idx] = {word[idx]}
            elif letter == "Y":
                try: word_vector[idx].remove(word[idx])
